...
import template from './dev.add.articles.html';

export const DevAddArticlesComponent = {
	template,
	controller: class DevAddArticlesComponent {
		constructor(DevAddArticlesService) {
      this.DevAddArticlesService = DevAddArticlesService;
      ...
      this.i = 0;
		...
		createArticles() {
				this.DevAddArticlesService.createArticle(this.articles).then((res) => {
          this.i++;
          if(this.i < this.numOfArticles) {
            this.createArticles();
          } else {
            this.i = 0;
            alert('Done');
